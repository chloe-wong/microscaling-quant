"""Element quantization to an arbitrary float(e, m) format: `rounding_mode` picks the rounding rule, `grid`
picks the set of representable values.

    grid="qtorch"   qtorch 0.2.0 float_quantize semantics (MXQuant as shipped). Biased exponent clipped to
                    [127-L, 127+L], L = 2^(e-1)-1: top exponent reserved (e4m3 max 240), one binade of "fake
                    subnormals" at 2^-L with full mantissa, below 2^(-L-1) flush to zero, overflow saturates.
                    With rounding_mode="ties_away" this is bit-identical to qtorch.quant.float_quantize(..., "nearest").
    grid="ieee"     IEEE-like format with bias 2^(e-1)-1, normals in [emin, emax] = [1-bias, bias], TRUE
                    subnormals with step 2^(emin-m), overflow -> Inf, NaN/Inf propagate. This is hardfloat's
                    RoundAnyRawFNToRecFN, i.e. the mesh lane accumulators; with rounding_mode="rne" it is
                    bit-identical to the gemmini golden `fp_quantize_rne`. (8, 7) is bf16.
    grid="ocp"      The OCP MX element formats (E4M3, E5M2, E3M2, E2M3, E2M1 only): same emin and subnormals as
                    ieee, but the top exponent holds normals (E4M3 max 448) and values beyond max_norm SATURATE.
                    Bit-identical to microxcaling `_quantize_elemwise(saturate_normals=True, allow_denorm=True)`
                    with round nearest/even/floor = ties_away/rne/truncate. These are MX-Gemmini's operand codes.

`rounding_mode` is one of mxq.rounding.MODES: "ties_away" | "rne" | "truncate".
Input is cast to float32 unless it is float64, in which case the ieee grid rounds from float64 directly
(needed by exact_add, whose exact sum may not fit float32). Output is float32.
(8, 23) is float32 itself: a float32 input is returned unchanged on every grid; a float64 input is rounded
to float32 with `rounding_mode` (ieee path).
"""
import torch

from .. import rounding
from .formats import FORMATS

__all__ = ["quantize", "bias", "GRIDS"]

GRIDS = ("qtorch", "ieee", "ocp")
_U32 = 0xFFFFFFFF   # keeps a float32 bit pattern inside 32 bits after int64 arithmetic


def bias(e: int) -> int:
    """Exponent bias of a float with e exponent bits: 2^(e-1) - 1."""
    return (1 << (e - 1)) - 1


def quantize(z: torch.Tensor, e: int, m: int, rounding_mode: str = "ties_away", grid: str = "qtorch") -> torch.Tensor:
    if rounding_mode not in rounding.MODES:
        raise ValueError(f"rounding_mode must be one of {rounding.MODES}, got {rounding_mode!r}")
    if grid not in GRIDS:
        raise ValueError(f"grid must be one of {GRIDS}, got {grid!r}")
    if (e, m) == (8, 23):                                   # float32 itself
        if z.dtype == torch.float64:
            return _ieee(z, 8, 23, rounding_mode)           # float64 -> float32 with the requested rule
        return z.detach().to(torch.float32).clone()
    if not (1 <= e <= 8 and 1 <= m <= 22):
        raise ValueError(f"unsupported widths e={e}, m={m}")
    return {"qtorch": _qtorch, "ieee": _ieee, "ocp": _ocp}[grid](z, e, m, rounding_mode)


def _qtorch(z, e, m, rounding_mode):
    """qtorch's float_kernel: round the float32 bit pattern at m fraction bits, then clip_exponent."""
    z32 = z.detach().to(torch.float32).contiguous()
    bits = z32.view(torch.int32).to(torch.int64) & _U32
    q = rounding.round_bits(bits, m, rounding_mode)

    L = bias(e)
    min_store, max_store = 127 - L, 127 + L
    exp_store = (q >> 23) & 0xFF
    sign = bits & 0x80000000
    max_num = sign | (max_store << 23) | (0x007FFFFF & ~((1 << (23 - m)) - 1))
    min_num = sign | (min_store << 23)
    middle = (min_store - 1) << 23
    q_mag = q & 0x7FFFFFFF

    q = torch.where(exp_store > max_store, max_num, q)
    below = exp_store < min_store
    q = torch.where(below & (q_mag > middle), min_num, q)
    q = torch.where(below & ~(q_mag > middle), torch.zeros_like(q), q)
    q = torch.where(q >= 1 << 31, q - (1 << 32), q).to(torch.int32)
    return q.view(torch.float32)


def _ieee(z, e, m, rounding_mode):
    """Scaled-integer rounding: step = 2^(max(E, emin) - m), k = |x|/step (exact), round k, multiply back.
    Subnormals fall out because step stops shrinking at emin; a carry past 2^(m+1) is handled by the
    overflow test on the value itself."""
    x = z.detach()
    x = x if x.dtype == torch.float64 else x.to(torch.float32)
    b = bias(e)
    emin, emax = 1 - b, b

    ax = x.abs()
    finite_nz = torch.isfinite(x) & (ax != 0)
    _, exp = torch.frexp(torch.where(finite_nz, ax, torch.ones_like(ax)))
    E = (exp - 1).to(x.dtype)                                     # unbiased exponent, exact
    step = torch.exp2(torch.clamp(E, min=float(emin)) - m)
    k = rounding.round_int(ax / step, rounding_mode)              # ax/step is exact (step is a power of two)
    val = k * step
    val = torch.where(val >= 2.0 ** (emax + 1), torch.full_like(val, float("inf")), val)

    out = torch.where(finite_nz, torch.copysign(val, x), x)      # NaN, Inf, +-0 pass through
    if e < 8:                                                     # golden scalar path: underflow -> +0.0;
        out = torch.where(finite_nz & (val == 0), torch.zeros_like(out), out)   # its e=8 bit path keeps the sign
    return out.to(torch.float32)


def _ocp(z, e, m, rounding_mode):
    """OCP element grid: ieee's emin and subnormal step, emax/max_norm from the format table, saturate.
    Zero-sign follows microxcaling: an exact +-0 input becomes +0.0; a negative value that rounds to zero
    becomes -0.0. Inf and NaN pass through."""
    f = next((f for f in FORMATS.values() if (f.e, f.m) == (e, m)), None)
    if f is None:
        raise ValueError(f"grid='ocp' is defined only for the OCP element formats, not (e={e}, m={m})")
    x = z.detach()
    x = x if x.dtype == torch.float64 else x.to(torch.float32)
    emin = 1 - bias(e)

    ax = x.abs()
    finite_nz = torch.isfinite(x) & (ax != 0)
    _, exp = torch.frexp(torch.where(finite_nz, ax, torch.ones_like(ax)))
    E = (exp - 1).to(x.dtype)
    step = torch.exp2(torch.clamp(E, min=float(emin)) - m)
    val = rounding.round_int(ax / step, rounding_mode) * step
    val = torch.clamp(val, max=f.max_norm)                       # saturate_normals

    out = torch.where(finite_nz, torch.copysign(val, x), x)      # Inf, NaN pass through; -tiny -> -0.0
    out = torch.where(x == 0, torch.zeros_like(out), out)        # exact +-0 -> +0.0
    return out.to(torch.float32)
