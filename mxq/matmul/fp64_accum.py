...
def fp64_accum(P_A: torch.Tensor, X_A: torch.Tensor, P_B: torch.Tensor, X_B: torch.Tensor,
               block: int = _blocks.BLOCK) -> torch.Tensor:
    A = _blocks.dequantize(P_A, X_A, axis=0, block_size=block).to(torch.float64)
    B = _blocks.dequantize(P_B, X_B, axis=0, block_size=block).to(torch.float64)
