"""The systolic column, numerically: MXQuant `_simulate_atw` loop (eval_complete.py:547-579) with the three
...
def systolic(P_A: torch.Tensor, X_A: torch.Tensor, P_B: torch.Tensor, X_B: torch.Tensor,
    ...
    """Y = Aᵀ·B. A: K×M codes, X_A: ceil(K/block)×M scales; B: K×N, X_B: ceil(K/block)×N. Y: M×N float32.
    ...
    `window` must divide `block`, otherwise a window would straddle two scale blocks.
    """
    K, M, N = check_operands(P_A, X_A, P_B, X_B, block)
    check_schedule(schedule, window)
    if block % window != 0:
        raise ValueError(f"window {window} must divide block {block}")
    C = torch.zeros((M, N), dtype=torch.float32, device=P_A.device)

    for g in range(0, K, block):
        g_end = min(g + block, K)
        scales = scale_map(X_A, X_B, g // block)
